...
const newTaskOwner = document.querySelector("#newTaskOwner");
...
function editTask(taskId) {
    const editing = tasks.find(function (task) {
        return task.id === taskId;
    });
    if (!editing)
        return;
    let newTask = prompt("Edit task title: ", editing.title);
    if (!newTask)
        return;
    editing.title = newTask.trim();
    refreshTaskViews();
}
